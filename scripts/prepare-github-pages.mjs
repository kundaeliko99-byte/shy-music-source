...
<html lang="en" class="dark">
  ...
  <body>
    <main class="wrap">
      ...
      <section class="card">
        ...
        <form id="password-form" class="hidden">
          ...
          <p class="foot"><button class="link hidden" id="use-code" type="button">Use a code instead</button></p>
        </form>
    ...
    <script>
      ...
      $("signin-form").onsubmit = (event) => { event.preventDefault(); const value = $("contact").value; const error = valid(value, method); if (error) return showNotice(error); identifier = normalize(value, method); if (failCount(method, identifier) >= MAX_FAILED_SIGNINS) return showNotice(supportMessage(identifier, method)); otpPurpose = "signin"; $("password-title").textContent = "Enter Your Password"; $("password-submit").textContent = "Log in"; $("forgot").classList.remove("hidden"); $("use-code").classList.add("hidden"); $("password").autocomplete = "current-password"; $("password").placeholder = "Enter your password"; if ($("remember").checked) localStorage.setItem("shy.auth.remembered", JSON.stringify({ contactMethod: method, contactValue: value })); $("password-id").textContent = mask(identifier); show("password-form"); $("password").focus(); };
      $("password-form").onsubmit = async (event) => { event.preventDefault(); try { requireClient(); const password = $("password").value; if (otpPurpose === "phone-reset") { const { error } = await client.auth.updateUser({ password }); if (error) throw error; location.href = BASE_PATH; return; } const args = method === "email" ? { email: identifier, password } : { phone: identifier, password }; const { error } = await client.auth.signInWithPassword(args); if (error) throw error; clearFail(method, identifier); location.href = BASE_PATH; } catch (e) { const attempts = bumpFail(method, identifier); const triesLeft = MAX_FAILED_SIGNINS - attempts; showNotice(attempts >= MAX_FAILED_SIGNINS ? supportMessage(identifier, method) : (e.message || "Could not sign in.") + " " + triesLeft + " " + (triesLeft === 1 ? "try" : "tries") + " left before support is required."); } };
      $("use-code").onclick = () => showNotice("Code sign-in is turned off for now. Use your password to sign in.");
